...
let slideSiete = {
   ...
   updateInputComplications: function () {
      ...
      inputs.forEach(input => {
         ...
         input.value = slideSiete.formatToFloatString(probabilidades[index - 1][riesgo]);
      });
   },
   ...
   actualizarInputs: function() {
      slideSiete.updateInputComplications();
      const customAlert = document.querySelector('custom-alert.block');
      if (customAlert) {
         setTimeout(() => {
   ...
   validarForm: function() {
      const validateComplications = slideSiete.validateComplications();
